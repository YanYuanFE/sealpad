    ...
    <div className="space-y-8">
      ...
      <div className="space-y-4">
        ...
        <p className="text-slate-600 max-w-3xl">
          SealPad is a confidential token-sale platform built on the Zama
          FHEVM. Contribution amounts and Dutch auction bids stay encrypted
          on-chain until the sale ends — only the final clearing price and
          your own allocation become public.
        </p>
      ...
      <DocsSection
        ...
        eyebrow="01 — OVERVIEW"
        icon={
          <Lightning size={18} weight="duotone" className="text-brand-500" />
        }
        title="What it is"
      >
        <p>
          ...
          investment amount, and a uniform clearing price is discovered after
          the sale ends.
        </p>
        <p>
          The privacy guarantee comes from FHEVM: contributions and bids are{" "}
          <Code>euint64</Code> ciphertexts, never decrypted on-chain mid-sale.
          When the sale finalizes, only the values the protocol actually needs
          (per-user contribution and the running total) are made publicly
          decryptable, and the KMS proof is verified on-chain before any
          settlement math runs.
        </p>
      </DocsSection>
      ...
      >
        <ol className="space-y-2 list-decimal list-inside text-sm marker:text-brand-500 marker:font-semibold">
          ...
          <li>
            ...
            after <Code>endTime</Code>. The contract publishes ciphertexts for
            KMS to decrypt.
          </li>
          <li>
            ...
            receives total payment; unsold tokens are returned to the creator.
          </li>
          <li>
        ...
        <Callout title="Why two phases?">
          FHEVM decryption is asynchronous: the contract can mark a ciphertext
          publicly decryptable, but the actual plaintext and KMS proof are
          produced off-chain and submitted in a follow-up call. The split
          between <Code>finalize</Code> and <Code>settle*</Code> reflects
          that.
        </Callout>
      </DocsSection>
      ...
      <DocsSection
        ...
        icon={<Coins size={18} weight="duotone" className="text-brand-500" />}
        ...
        <div className="grid md:grid-cols-2 gap-4">
          <Card>
            ...
            <CardContent className="text-sm space-y-2 text-slate-600">
              <p>
                Creator sets a single <Code>price</Code>. Each contribution is
                FHE-clamped to the user's deposit and added to an encrypted
                running total.
              </p>
              <p>
                If the decrypted total exceeds the hard cap, every participant
                is scaled down proportionally so the raise lands exactly on
                the cap.
              </p>
            </CardContent>
          </Card>
          <Card>
            ...
            <CardContent className="text-sm space-y-2 text-slate-600">
              <p>
                ...
                picks their own <Code>bidPrice</Code> (public) ≥ floor and an
                encrypted investment amount.
              </p>
              <p>
                After the sale ends, bids are processed from highest price
                down. The lowest accepted price becomes the uniform clearing
                price — everyone above pays the same. Bidders at the clearing
                price share the remaining tokens pro-rata.
              </p>
            </CardContent>
          </Card>
        </div>
      ...
      <DocsSection
        ...
        icon={
          <ShieldCheck size={18} weight="duotone" className="text-brand-500" />
        }
        title="Independent deposit pool"
      >
        <p>
          A user's <strong>deposit</strong> (public) is the upper bound of how
          much they can spend. The actual spend — the contribution or bid
          amount — is encrypted and FHE-clamped to that bound. Updating a
          contribution moves <em>only the ciphertext</em>; no funds change
      ...
      >
        <div className="space-y-3">
          <Step n={1} title="Connect on Sepolia">
            SealPad runs on {REQUIRED_CHAIN_LABEL} only. Make sure your wallet
            is on chain {REQUIRED_CHAIN_ID}; the app will warn you otherwise.
          </Step>
          ...
          <Step n={3} title="Submit an encrypted contribution or bid">
            ...
            <Code>contribute</Code> (Fixed Price) or <Code>bid</Code> (Dutch).
            Updating later moves no tokens.
          </Step>
          <Step n={4} title="Wait for finalize + settle">
            After <Code>endTime</Code>, anyone can call <Code>finalize</Code>.
            Once KMS produces the decryption proof, anyone submits it via{" "}
            <Code>settleFixed</Code> or <Code>settleDutch</Code>.
          </Step>
      ...
      >
        <p>
          ...
          page locks <Code>saleAmount</Code> sale tokens into the contract and
          registers the parameters below.
        </p>
...
settleFixed(saleId, decryptedValues, proof)
  ...
  → returns unsold tokens to creator`}</pre>
      ...
      <DocsSection
        ...
        icon={<Hash size={18} weight="duotone" className="text-brand-500" />}
        title="The numbers to remember"
      >
        <div className="grid md:grid-cols-3 gap-4">
          <KeyValue label="MAX_PARTICIPANTS" value="50" />
          <KeyValue label="Sale-token scale" value="10**decimals(), per sale" />
          <KeyValue label="Pay-token math" value="uint64 (FHE constraint)" />
          <KeyValue label="Sale-token decimals" value="Any (read at create)" />
          <KeyValue
